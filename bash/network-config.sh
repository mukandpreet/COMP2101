...
dataForAdd=$(ip a |awk '/: e/{gsub(/:/,"");print $2}')
dataForIPA=$(ip a |awk '/: e/{gsub(/:/,"");print $2}')
dataForHost=$(ip a s $dataForIPA|awk '/inet /{gsub(/\/.*/,"");print $2}')
myExtIP=$(curl -s icanhazip.com)
routerAdd=$(netstat -nr | awk '$1 == "0.0.0.0"{print$2}')
cat <<EOF
Hostname        : $(hostname)
LAN Address     : $(ip a s $dataForAdd|awk '/inet /{gsub(/\/.*/,"");print $2}')
LAN Hostname    : $(getent hosts $dataForHost| awk '{print $2}')
External IP     : $myExtIP
External Name   : $(getent hosts $myExtIP | awk '{print $2}')
Router Address	: $routerAdd
Router Name	: $(getent hosts $routerAdd |awk '{print $2}')
EOF
